...
This project explores the use of **Google Antigravity** to build a production-grade application end to end.
While earlier coding agents were capable of generating functional code, they often struggled with large-scale structural changes. Antigravity handled this challenge exceptionally well, successfully transforming a linear processing pipeline into a fully stateful **LangGraph** workflow.
More than 98% of the codebase was produced by the Antigravity agent. Human involvement focused on architectural direction, prompt design, and maintaining production-grade engineering standards.


Recipe Helper is an AI-powered culinary companion that turns photos of your pantry into delicious meal ideas. The application leverages **LangGraph** orchestration and **situational context** (weather and time) to provide a more dynamic and interactive user experience. The application executes as a stateful graph with human-in-the-loop pauses and enriches model prompts with live weather data and the current time in Dublin.
